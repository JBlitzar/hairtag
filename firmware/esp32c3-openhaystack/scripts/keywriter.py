...
def find_keyfile(input_dir):
    ...
    if not keyfiles:
        print(
            f"Error: No keyfile found in the {input_dir} directory. Please place a file ending with '_keyfile' in the input directory."
        )
        sys.exit(1)

    if len(keyfiles) > 1:
        print(
            "Multiple keyfiles found. Please ensure only one keyfile exists in the input directory."
        )
        for kf in keyfiles:
...
def process_binary_keyfile(file_path):
    ...
    try:
        ...
        if len(key_data) < 28:
            print(
                f"Error: Key file too short ({len(key_data)} bytes), expected at least 28 bytes"
            )
            return None
        ...
        if len(key_data) == 28:
            ...
            if key_count > 0 and data_size % 28 == 0 and data_size // 28 == key_count:
                ...
                return [
                    key_data[i : i + 28]
                    for i in range(0, len(key_data), 28)
                    if i + 28 <= len(key_data)
                ]
                ...
                if line:
                    try:
                        ...
                        if len(key_bytes) == 28:
                            ...
                            print(
                                f"Warning: Key has incorrect length {len(key_bytes)} bytes, expected 28 bytes"
                            )
                    except Exception as e:
...
def write_key_partition(port, keys_file, deployed_dir):
    ...
    try:
        ...
        print(
            "Error: esptool module not found. Please install it with 'pip install esptool'"
        )
    except Exception as e:
